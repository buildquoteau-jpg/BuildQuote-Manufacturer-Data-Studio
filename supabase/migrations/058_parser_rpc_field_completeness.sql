-- BuildQuote Data Studio — Parser RPC field completeness
-- Migration: 058_parser_rpc_field_completeness.sql
--
-- The 2026-07-18 pipeline audit found the inverse of the migration-057 bug
-- class: columns that EXIST in the live schema and are populated by the
-- parser's plan payload, but that insert_parser_output_plan_v1 never lists in
-- its INSERT statements — so the values were extracted, paid for, and then
-- silently discarded (no error, unlike the loud 42703 that 057 fixed).
--
-- Fixes, relative to the 057 function body:
--   1. staged_systems: insert `australian_made` (boolean, nullable — null
--      means "not stated in the catalogue", per migration 021 / the Stage 1
--      prompt contract). The parser has emitted this field since SYSTEM_FIELDS
--      was introduced; it never reached the table.
--   2. staged_system_profiles: insert `sheet_format` (text). Requested by the
--      Stage 1 prompt and passed by PROFILE_FIELDS; never reached the table.
--   3. staged_system_components.role: the null-fallback becomes 'component',
--      matching migration 017 which retired the required/optional taxonomy
--      and set the table default to 'component'. The old COALESCE(…,
--      'required') resurrected a retired value whenever the parser omitted a
--      role.
--
-- A round-trip test (supabase/tests/012_06_round_trip_field_loss.sql) now
-- guards this class: it inserts a plan with every field non-null and asserts
-- every value survives into the tables. Run it after applying this migration.
--
-- CREATE OR REPLACE preserves the existing ACL (locked down by migrations
-- 010 + 011, granted to service_role by 014) — no new grants needed.
--
...
BEGIN
  ...
  --    Role fallback is 'component' per migration 017 (free-text role,
  --    constrained taxonomy retired).
  -- ================================================================
