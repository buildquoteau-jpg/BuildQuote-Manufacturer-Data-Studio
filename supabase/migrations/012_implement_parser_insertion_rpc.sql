-- BuildQuote Data Studio — Implement parser insertion RPC
-- Migration: 012_implement_parser_insertion_rpc.sql
--
-- Replaces the dry RPC shell (migration 010) with real insertion logic.
-- The function remains locked down to postgres/owner only (migrations 010 + 011).
-- No new grants are added here — CREATE OR REPLACE preserves the existing ACL.
--
-- Implements the full 7-table insert for a ParserInsertionPlan JSON payload:
--   1. staged_systems              — builds system UUID map
--   2. staged_system_profiles      — FK: staged_system_id resolved via temp key
--   3. staged_components           — builds component UUID map
--   4. staged_system_colours       — FK: staged_system_id resolved via temp key
--   5. staged_system_components    — FK: staged_system_id + staged_component_id
--   6. field_verifications         — entity_id resolved from UUID maps by entity_type
--   7. parser_field_evidence       — extraction_run_id FK; always append-only
--
-- V1 behaviour (per docs/parser-insertion-adapter-design.md):
--   §10.4  Append-only per extraction run — no cross-run deduplication.
--   §10.5  Unresolved required temp-key links raise EXCEPTION and abort.
--   §10.7  source_page_id left null (resolved from source_page_number in a later version).
--   §10.6  staged_*.verification_status = 'pending_review'
--          field_verifications.status   = 'pending'
--
-- Schema notes confirmed against live local schema (migrations 001–011):
--   - staged_system_profiles has NO extraction_confidence column.
--   - staged_system_colours  has NO extraction_confidence column.
--   - parser_field_evidence.extraction_run_id is NOT NULL with FK → extraction_runs.id.
--   - field_verifications and parser_field_evidence source_chunk_id have FK → document_chunks.id.
--     Caller is responsible for providing valid IDs or null.
--
