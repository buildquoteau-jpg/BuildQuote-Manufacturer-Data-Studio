            ...
            <div key={route ?? 'all'}>
              {isSplit && (
                <div style={{
                  ...
                  paddingLeft: '4px', borderLeft: '3px solid #d1d5db',
                }}>
                  {label}
                </div>
              )}
      ...
      >
        ...
        <div style={{
          ...
          {!system.hero_image_url && (
            <span style={{ fontSize: '22px', fontWeight: 800, color: '#94a3b8', fontFamily: 'monospace' }}>
              {system.product_code}
            </span>
          )}
          <span style={{
        ...
        <div style={{ padding: '24px 24px 28px', flex: 1 }}>
          <h2 style={{ margin: 0, fontSize: '22px', fontWeight: 800, color: '#0f172a', lineHeight: 1.2 }}>
            {system.name}
          </h2>
          {system.product_code && (
            <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#9ca3af', fontFamily: 'monospace', letterSpacing: '0.04em' }}>
              {system.product_code}
            </p>
          )}
          {system.description && (
            <p style={{ margin: '12px 0 0', fontSize: '14px', color: '#374151', lineHeight: 1.65 }}>
              {system.description}
            </p>
          )}
          ...
          {system.system_profiles.length === 0 && mode === 'rfq' && draftId && (
            <div style={{ marginTop: '18px', paddingTop: '16px', borderTop: '1px solid #e5e7eb' }}>
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '13px', fontWeight: selectedBase ? 700 : 600, color: selectedBase ? '#0f2d3d' : '#111827', lineHeight: 1.3 }}>
                    {system.name}
                  </div>
          ...
          <div style={{ marginTop: '24px', paddingTop: '20px', borderTop: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {mode === 'rfq' ? (
              !draftId ? (
                ...
                >
                  {adding ? 'Adding…' : selCount > 0 ? `Add ${selCount} selected to RFQ →` : 'Select profiles or components above'}
                </button>
              )
            ) : (
              <button
