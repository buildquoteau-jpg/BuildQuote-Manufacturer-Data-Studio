...
// The Cover — the first screen of the card object. Its only job is to
// answer "should I open this?" Real photography only: hero_image_url +
// gallery_images, deduplicated (the two are the same asset for Apex PLUS
// today) and capped at 10, swipeable via native horizontal scroll-snap — no
// carousel dependency. The statement is the first real sentence of
// system.description.
//
// No attribute pills here — moved to Attributes and Information per
// Melia's direction, so they don't compete with the hero photography.
// Turning the page to Reveal 1 is the shared bottom bar's job now (see
// SystemCardV2Experience.tsx) — the Cover itself only supplies the photo,
// identity and Share.
//
// Tapping a gallery photo opens it full screen (close / prev / next /
// counter) — parity with the real card's existing lightbox, which Melia
// pointed at directly. `position: fixed` with no transformed ancestors in
// this tree, so it escapes the card frame's own overflow:hidden/rounded
