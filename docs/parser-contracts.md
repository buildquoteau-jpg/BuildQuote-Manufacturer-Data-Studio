...
| 6 | How should the insertion layer consistently populate both `name` and `profile_name` on `staged_system_profiles`? Decision already made: `profile_name` is the short profile/variant label; `name` remains the full descriptive profile name where the staged schema supports it; the parser may output both; the insertion layer must avoid silently copying one into the other unless fallback behaviour is explicitly documented. | Insertion layer implementation detail |
